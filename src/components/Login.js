...
const Login = ()=>{
    ...
    return <div>
        ...
        <form className="p-12 text-white bg-black bg-opacity-80 rounded-lg absolute my-36 right-0 left-0 mx-auto w-3/12">
            ...
            <div className="p-4 m-4">{isSignInForm ? 'New to Netflix?' : 'Already you have an account?'} <b className="cursor-pointer text-xl underline" onClick={toggleSignInForm}>{isSignInForm ? 'Sign Up' : 'Sign In'} Now.</b></div>
        </form>
    </div>
}
